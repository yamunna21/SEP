...
@section('content')
    ...
    <form action="{{route ('updateApproval')}}" method="POST">
    @csrf


            <div class="form-group">
                <input hidden type="text" class="form-control" name="id" id="id" value="{{$accept->id}}">
            </div>

    <div class="form-group">
        <label for="username">Username</label>
        <input readonly type="text" class="form-control" name="username" id="username" value="{{$accept->name}}">
    </div>

        <div class="form-group">
            <label for="devicetype">Device Type</label>
            <input readonly type="text" class="form-control" name="devicetype" id="devicetype" value="{{$accept->devicetype}}">
        </div>

        <div class="form-group">
            <label for="model">Device Model</label>
            <input readonly type="text" class="form-control" name="model" id="model" value="{{$accept->model}}">
        </div>

        <div class="form-group">
            <label for="deviceos">Device OS</label>
            <input readonly type="text" class="form-control" name="deviceos" id="deviceos" value="{{$accept->deviceos}}">
        </div>

        <div class="form-group">
            <label for="repairdetails">Repair Details</label>
            <input readonly type="text" class="form-control" name="repairdetails" id="repairdetails" value="{{$accept->repairdetails}}">
        </div>

        <div class="form-group">
            <label for="estimated_cost">Estimated Cost</label>
            <input type="text" class="form-control" name="estimated_cost" id="estimated_cost" value="{{$accept->estimated_cost}}" required>
        </div>


        <input hidden type="text" name="s_approval" value="Accepted">
        <input hidden type="text" name="staffincharge" value="{{Auth::user()->username}}">
        <button class="btn btn-success" >Save Info</button>

    </form>

{{--                            <form method="POST" action="{{ route('updateApproval')}}"/>--}}
{{--                                @method('PUT')--}}
{{--                                @csrf--}}
